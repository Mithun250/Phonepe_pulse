    ...
    if Type == "Users":
        if select == "Top 10 Districts based on the Transaction Amount" or select == "Least 10 Districts based on the Transaction Amount" or select=="Top 10 Districts based on the Transaction count" or select == "Least 10 Districts based on the Transaction count":
            ...
            with col1:
                ...
                suresh.execute(f"SELECT district, SUM(RegisteredUser) AS Total_Users, SUM(AppOpens) AS Total_Appopens FROM map_user WHERE year = {Year} AND quarter = {Quarter} GROUP BY district ORDER BY Total_Users DESC LIMIT 10")
                
                df = pd.DataFrame(suresh.fetchall(), columns=["District", "Total_Users", "Total_Appopens"])
            ...
            with col2:
                st.markdown("### :violet[State]")
                suresh.execute(f"SELECT state, SUM(Registereduser) AS Total_Users, SUM(AppOpens) AS Total_Appopens FROM map_user WHERE year = {Year} AND quarter = {Quarter} GROUP BY state ORDER BY Total_Users DESC LIMIT 10")
                df = pd.DataFrame(suresh.fetchall(), columns=["State", "Total_Users", "Total_Appopens"])
            ...
            with col1:
                st.markdown("### :violet[District]")
                suresh.execute(f"SELECT district, SUM(RegisteredUser) AS Total_Users, SUM(AppOpens) AS Total_Appopens FROM map_user WHERE year = {Year} AND quarter = {Quarter} GROUP BY district ORDER BY Total_Users ASC LIMIT 10")
                df = pd.DataFrame( suresh.fetchall(), columns=["District", "Total_Users", "Total_Appopens"])
                df.Total_Users = df.Total_Users.astype(float)
            ...
            with col2:
                st.markdown("### :violet[State]")
                suresh.execute(f"SELECT state, SUM(Registereduser) AS Total_Users, SUM(AppOpens) AS Total_Appopens FROM map_user WHERE year = {Year} AND quarter = {Quarter} GROUP BY state ORDER BY Total_Users ASC LIMIT 10")
                df = pd.DataFrame(suresh.fetchall(), columns=["State", "Total_Users", "Total_Appopens"])
