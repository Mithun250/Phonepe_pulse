            ...
            with col2:
                ...
                with col2:
                    ...
                    if (Year == 2022 and Quarter in [2, 3, 4]) or (Year == 2023 and Quarter in [1,2,3,4]):
                        ...
                        suresh.execute(f"SELECT brands, SUM(count) AS Total_Count, AVG(percentage)*100 AS Avg_Percentage FROM agg_user WHERE year = {Year} AND quarter = {Quarter} GROUP BY brands ORDER BY Total_Count ASC LIMIT 10")
                        df = pd.DataFrame( suresh.fetchall(), columns=["Brand", "Total_Users", "Avg_Percentage"] )
